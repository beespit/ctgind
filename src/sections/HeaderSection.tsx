...
// Custom hook to get cart quantity from our custom cart implementation
function useCartQuantity() {
  ...
  const fetchCartQuantity = async () => {
    ...
    try {
      ...
      if (result.data?.cart?.totalQuantity) {
        setTotalQuantity(result.data.cart.totalQuantity);
      } else {
        setTotalQuantity(0);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Listen for storage changes to update cart quantity when items are added
  ...
  return (
    <header className={`${router.pathname === '/products/[handle]' ? ' fixed top-0 z-[13] w-[100%]' : ''}`}>
      ...
      <ul className="grid w-full grid-cols-small md:grid-cols-main xl:grid-cols-big">
        ...
        <NextLink href='/about' className="hover:bg-black outline-offset-[-1px] font-sans ${surt.variable}"><li className={`h-full p-[15px] outline outline-2 outline-offset-[-1px] hover:text-white hover:outline-black ${router.pathname === '/about'  ? 'bg-[#000] text-white outline-black' : ''}`}>About</li></NextLink>
